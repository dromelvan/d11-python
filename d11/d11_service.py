...
class D11Service:
    ...
    def generate_d11_fixtures(self):
        """
        Generate D11 fixtures. Depends on having 20 teams in the d11_team_season_stat table for the pending season with rankings 1-20
        """
    ...
    values((select d11_team_id from $schema.d11_team_season_stat where ranking = {} and season_id = (select max(id) from $schema.season)),
        ...
        # Shuffle matches within each match day to avoid always having the same team first
        ...
        with open('d11_fixtures.sql', 'w') as file:
            file.write(output)
